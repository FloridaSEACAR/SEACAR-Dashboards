...
db_thresholds <- SEACAR::DB_Thresholds %>% select(Habitat, ParameterName, Units)
setDT(db_thresholds)
...
for(h in names(habitats)){
  ...
  data_directory[[habitat]][["MAPrograms"]] <- data %>%
    group_by(ProgramID, ProgramName, AreaID, ManagedAreaName) %>% summarise() %>%
    ...
    SEACAR::clean_managed_areas(type = "ma") %>% distinct() %>%
    left_join(MA_All[, c("AreaID", "ManagedAreaName", "ShortName")], # Add ShortName for display on dashboard
              join_by(AreaID==AreaID, ManagedAreaName==ManagedAreaName)) %>%
    select(-AreaID)
  ...
  ma_data <- data %>%
    group_by(ProgramLocationID, ProgramID, ProgramName, LocationID, OriginalLatitude, OriginalLongitude) %>%
    summarise(n_data = n(),
  ...
  mapData <- pre_data %>%
    mutate(popup = paste("<b>ProgramID:</b> ", ProgramID,
                         "<br> <b>ProgramName:</b> ", ProgramName,
                         "<br> <b>LocID:</b> ", LocationID,
                         "<br> <b>ProgLocID:</b> ", ProgramLocationID,
                         "<br> <b>N_Data:</b> ", n_data,
                         "<br> <b>Years:</b> ", years,
           ...
           habitat = habitat,
           rad = sqrt(n_data)/8) %>%
    rename(lat = OriginalLatitude, lon = OriginalLongitude)
  ...
  rm(programYears, programs, ma_data)
}
...
skt_stats_cont <- fread("../../SEACAR_Trend_Analyses/WQ_Cont_Discrete/output/WQ_Continuous_All_KendallTau_Stats.txt") %>%
...
data_output_disc <- setDT(readRDS(str_subset(disc_files, "data_output_disc")))
...
original_data_loc <- c("Coastal_Wetlands/output/",
                       ...
                       "Nekton/output/",
                       "Oyster/output/ManagedAreaName/", "SAV/output/website/")
...
# Create offline version of files from SEACAR package
saveRDS(SEACAR::DB_Thresholds, file = "rds/Database_Thresholds.rds")
saveRDS(setDT(SEACAR::TableDescriptions), file = "rds/TableDescriptions.rds")
# Figure captions to implement within plot_df below
...
plot_info <- list(
  ...
  multiplot = list(
    ...
    habitat = "Submerged Aquatic Vegetation",
    FigureCaption = FigureCaptions[str_detect(HabitatName, "Submerged") & PlotType=="multiplot", FigureCaptions]
  ),
  barplot_sp = list(
    ...
    habitat = "Submerged Aquatic Vegetation",
    FigureCaption = FigureCaptions[str_detect(HabitatName, "Submerged") & PlotType=="barplot", FigureCaptions]
  ),
  trendplot = list(
    title = "Median percent cover for ",
    alt = "Median Percent LME Trends",
    ...
    habitat = "Submerged Aquatic Vegetation",
    FigureCaption = FigureCaptions[str_detect(HabitatName, "Submerged") & PlotType=="trendplot", FigureCaptions]
  ),
  Oyster_Dens = list(
    ...
    habitat = "Oyster Reef",
    FigureCaption = FigureCaptions[str_detect(HabitatName, "Oyster") & ParameterName=="Density", FigureCaptions]
  ),
  Oyster_SH = list(
    ...
    habitat = "Oyster Reef",
    FigureCaption = FigureCaptions[str_detect(HabitatName, "Oyster") & ParameterName=="Shell Height", FigureCaptions]
  ),
  Oyster_PrcLive = list(
    ...
    habitat = "Oyster Reef",
    FigureCaption = FigureCaptions[str_detect(HabitatName, "Oyster") & ParameterName=="Percent Live", FigureCaptions]
  ),
  Nekton_SpeciesRichness = list(
    ...
    FigureCaption = FigureCaptions[str_detect(IndicatorName, "Nekton"), FigureCaptions]
  ),
  Coral_pc = list(
    ...
    habitat = "Coral Reef",
    FigureCaption = FigureCaptions[str_detect(HabitatName, "Coral") & IndicatorName=="Percent Cover", FigureCaptions]
  ),
  Coral_SpeciesRichness = list(
    ...
    habitat = "Coral Reef",
    FigureCaption = FigureCaptions[str_detect(HabitatName, "Coral") & IndicatorName=="Grazers and Reef Dependent Species", FigureCaptions]
  ),
  CoastalWetlands_SpeciesRichness = list(
    ...
    FigureCaption = FigureCaptions[str_detect(HabitatName, "Coastal") & IndicatorName=="Species Composition", FigureCaptions]
  )
)
...
# Function to apply trend icons
apply_trend_icons <- function(trendIcon, text){
  if(trendIcon==2){
    icon <- "insuff-data.png"
  } else if(trendIcon==3){
    icon <- "no-analysis-medium-icon.png"
  } else if(trendIcon==1){
    icon <- "increase-medium-icon.png"
  } else if(trendIcon==-1){
    icon <- "decrease-medium-icon.png"
  } else if(trendIcon==0){
    icon <- "no-trend-medium-icon.png"
  } else {
    icon <- "no-analysis-medium-icon.png"
  }
  icon_loc <- paste0("trendarrows/", icon)
  # tags$img(src = icon_loc, alt = text)
  paste0(
    '<img src = "', icon_loc, '" height = "30px" width = "30px" alt = "', text, '">'
  )
}
...
sav_trends <- fread("data/SAV_BBpct_LMEresults_All.txt", sep='|') %>%
  rowwise() %>%
  mutate(icon = apply_trend_icons(TrendIcon, TrendText),
         `Period of Record` = paste0(EarliestYear, " - ", LatestYear),
         LME_Intercept = round(LME_Intercept, 2),
         LME_Slope = round(LME_Slope, 2),
         p = round(p, 4)) %>%
  select(-c(TrendIcon, TrendText)) %>%
  filter(!Species=="No grass in quadrat") %>%
  rename(`Trend Status` = icon,
         `LME Intercept` = LME_Intercept, `LME Slope` = LME_Slope) %>%
  select("ManagedAreaName","Species","Trend Status","Period of Record","LME Intercept","LME Slope","p") %>%
  as.data.table()
...
oy_trends <- fread("data/Oyster_All_GLMM_Stats.txt", sep='|') %>%
  rowwise() %>%
  mutate(icon = apply_trend_icons(TrendIcon, TrendText),
         ModelEstimate = round(ModelEstimate, 2),
  ...
  select(ManagedAreaName, ParameterName, ShellType, HabitatType, SizeClass, icon, `Period of Record`, ModelEstimate,
         StandardError, CredibleInterval) %>%
  rename(`Shell Type` = ShellType, `Habitat Type` = HabitatType, `Size Class` = SizeClass,
         `Trend Status` = icon, `Estimate` = ModelEstimate,
...
coral_pc_trends <- fread("data/Coral_PC_LME_Stats.txt", sep='|') %>%
  ...
  ungroup() %>% rename(`LME Intercept` = LME_Intercept, `LME Slope` = LME_Slope,
                       `p` = LME_p, `Trend Status` = icon) %>%
  select(ManagedAreaName, `Trend Status`, `Period of Record`,
...
coral_sr_trends <- fread("data/Coral_SpeciesRichness_MA_Overall_Stats.txt", sep='|') %>%
  ...
  select(ManagedAreaName, `No. of Samples`, `No. Years with Data`, `Period of Record`,
         `Median No. of Taxa`, `Mean No. of Taxa`) %>% as.data.table()
...
nekton_trends <- fread("data/Nekton_SpeciesRichness_MA_Overall_Stats.txt", sep='|') %>%
  ...
  rename(`Gear Type` = GearType,`No. of Samples` = N_Data,
         `No. Years with Data` = N_Years, `Median No. of Taxa` = Median,
         `Mean No. of Taxa` = Mean) %>%
  select(ManagedAreaName, `Gear Type`, `No. of Samples`, `No. Years with Data`, `Period of Record`,
         `Median No. of Taxa`, `Mean No. of Taxa`) %>% as.data.table()

## Coastal Wetlands
cw_trends <- fread("data/CoastalWetlands_SpeciesRichness_MA_Overall_Stats.txt", sep='|') %>%
  mutate(`Period of Record` = paste0(EarliestYear, " - ", LatestYear)) %>%
  rename(`Species Group` = SpeciesGroup1, `No. of Samples` = N_Data,
         `No. Years with Data` = N_Years, `Median No. of Taxa` = Median, `Mean No. of Taxa` = Mean) %>%
  select(ManagedAreaName, `Species Group`, `No. of Samples`, `No. Years with Data`,
         `Period of Record`, `Median No. of Taxa`, `Mean No. of Taxa`) %>%
  as.data.table()
